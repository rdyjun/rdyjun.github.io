...
const blogPosts = [
  {
    ...
    "date": "2026-08-06",
    "url": "articles/page-handle-unique-constraint.html",
    "excerpt": "최근 개발 중 유니크 제약 관련 문제가 있었다. 구체적인 도메인은 밝힐 수 없어서, 여기서는 사용자 와 사용자 닉네임 으로 예를 들겠다. 해당 프로젝트에서 사용자가 약 100명 정도로 적었기 때문에 가벼운 조건 검사로 둘 수 있었지만, 해당 닉네임이 각 사용자의 서브…"
  },
  {
    "title": "첫 오픈소스 기여",
    "date": "2026-07-31",
    "url": "articles/cloudflare-tunnel-contribution.html",
    "excerpt": "오픈소스 기여는 오래전부터 해보고 싶었던 일 중 하나였다. 하지만 막상 시작하려고 하면 어느 저장소에서 무엇부터 해야 할지 감이 잡히지 않아 계속 미루고 있었다. 그러다 오픈소스 기여에 다시 관심이 생겨서, 평소에 직접 쓰는 서비스와 관련된 저장소를 찾아보기로 했다.…"
  },
  {
  ...
  },
  {
    ...
    "excerpt": "QueryDSL을 사용해서 각 행으로 분리된 권한 데이터를 List로 합치는 과정에서 아래와 같은 문제에 직면했다. 기존에 string_agg 방식으로 구현했을 때 발생한 문제는 아니고, transform 방식으로 변경하려다 발생한 문제이다. 기존 코드에서 QueryD…"
  },
  {
    "title": "JPA와 복합키",
    "date": "2025-03-28",
  ...
  },
  {
    ...
    "excerpt": "예정된 4주간의 네이버 부스트캠프 챌린지 과정이 모두 끝이났다. 다양한 학습 방식을 통해 학습에 몰입할 수 있었던 시간이었고, 언제 또 이만큼의 몰입을 해볼 수 있을 지 모르겠을 만큼 나를 갈아넣은 시간이었다고 생각한다. 챌린지 4주차 과정이 2024. 08. 09 일…"
  },
  {
  ...
  },
  {
    ...
    "excerpt": "로드 밸런싱은 여러 서버를 두고, 부하를 각 서버로 분산하기 위해 사용되는 기술이다. 이 로드 밸런싱의 종류와 특징을 알아볼 것이다."
  },
  {
